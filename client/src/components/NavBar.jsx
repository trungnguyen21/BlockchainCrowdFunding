  ...
  return (
    <div className='flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6'>
      <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
          <input type='text' placeholder='Search for campaign' className=' flex w-full font-monserrat font-normal bg-transparent outline-none text-[#fff] text-sm'/>
                ...
                {navlinks.map((link) => (
                  <li key={link.name} className={`flex p-4 ${isActive === link.name && 'bg-[#3a3a43]'}`}
                    onClick={() => {
                      ...
                      <p className={`ml-4 text-[#fff] font-monserrat font-normal text-sm ${isActive === link.name ? 'text-[#1dc071]' : 'text-[#808191]'}`}>{link.name}</p>
